...
import { Routes, Route, BrowserRouter, Link } from 'react-router-dom'; // Added Link
...
function findZero(puzzle: number[][]) {
  ...
  return [-1, -1]; // Should not happen in a valid puzzle
}
...
function* bfsStepGenerator( // Renamed from dfsStepGenerator
  start: number[][],
  goal: number[][],
  maxDepth = 30
) {
  ...
  const queue: { puzzle: number[][]; depth: number; from: string | null }[] = [ // Renamed from stack to queue
    { puzzle: clonePuzzle(start), depth: 0, from: null },
  ];
  const explored: string[] = [];
  let foundKey: string | null = null;

  while (queue.length > 0) { // Changed from stack.length
    yield {
      explored: [...explored],
      currentPath: [],
      found: !!foundKey,
      parent: { ...parent },
      foundKey,
      queueSnapshot: queue.map(s => ({ key: puzzleToString(s.puzzle), depth: s.depth, from: s.from })), // Renamed from stackSnapshot
      processing: false, 
    };

    const { puzzle, depth, from } = queue.shift()!; // Changed from stack.pop() to queue.shift()
    ...
    for (const neighborPuzzle of neighbors) { // Iterate neighbors in order for BFS
      const neighborKey = puzzleToString(neighborPuzzle);
      if (!visited.has(neighborKey) && !queue.find(s => puzzleToString(s.puzzle) === neighborKey)) { // Check queue instead of stack
        queue.push({ puzzle: neighborPuzzle, depth: depth + 1, from: key }); // Add to end of queue
      }
    }
  }
  yield {
    ...
    parent: { ...parent },
    foundKey,
    queueSnapshot: [], // Renamed from stackSnapshot
    processing: false,
  };
}
...
const PuzzleGrid = memo(function PuzzleGrid({ puzzle }: { puzzle: number[][] }) {
  return <PuzzleGridDisplay puzzle={puzzle} />; // Replaced with new component
});
    ...
    <div style={{ width: '100vw', height: '100vh' }}>
      ...
      >
        ...
        <MiniMap nodeColor={n => {
            ...
            if (nodeData?.style?.border?.includes('dashed')) return '#adb5bd'; 
